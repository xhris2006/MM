...
export async function DELETE(
  ...
  try {
    ...
    return NextResponse.json({
      success: true,
      message: 'Utilisateur et ses votes supprimés.'
    })
...
export async function PATCH(
  ...
  try {
    ...
    if (action === 'change_vote') {
      ...
      if (!newCandidate) {
        return NextResponse.json({ error: 'Candidat cible introuvable' }, { status: 404 })
      }
    ...
    if (action === 'add_vote') {
      ...
      if (alreadyVoted) {
        return NextResponse.json(
          { error: `Cet utilisateur a déjà voté en ${newCandidate.category}.` },
          { status: 409 }
        )
      }
